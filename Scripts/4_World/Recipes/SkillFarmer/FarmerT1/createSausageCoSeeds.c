class createSausageCoSeeds extends RecipeBase
{	
	override void Init()
	{
		m_Name = "Create ";
		...
		//INGREDIENTS
		//ingredient 1
		InsertIngredient(0,"SC_Cannabis_Alaskan_ThunderFuck_Seeds");//you can insert multiple ingredients this way
		...
		//ingredient 2
		InsertIngredient(1,"SausageCo_FarmersToolT1");//you can insert multiple ingredients this way
		...
		//result1
		AddResult("egOCSds");//add results here
		...
		// result2
		AddResult("egOCSds1");//add results here
	...
	// result3
		AddResult("egOCSds2");//add results here

		m_ResultSetFullQuantity[2] = false;//true = set full quantity, false = do nothing
		m_ResultSetQuantity[2] = 3;//-1 = do nothing
	...
	// result3
		AddResult("egOCSds3");//add results here

		m_ResultSetFullQuantity[3] = false;//true = set full quantity, false = do nothing
		m_ResultSetQuantity[3] = 3;//-1 = do nothing
	...
	// result4
		AddResult("egOCSds4");//add results here

		m_ResultSetFullQuantity[4] = false;//true = set full quantity, false = do nothing
		m_ResultSetQuantity[4] = 3;//-1 = do nothing
		...
		AddResult("egOCSds5");//add results here

		m_ResultSetFullQuantity[5] = false;//true = set full quantity, false = do nothing
		m_ResultSetQuantity[5] = 3;//-1 = do nothing
		m_ResultSetHealth[5] = -1;//-1 = do nothing
		m_ResultInheritsHealth[5] = 0;// (value) == -1 means do nothing; a (value) >= 0 means this result will inherit health from ingredient number (value);(value) == -2 means this result will inherit health from all ingredients averaged(result_health = combined_health_of_ingredients / number_of_ingredients)
		m_ResultInheritsColor[5] = -1;// (value) == -1 means do nothing; a (value) >= 0 means this result classname will be a composite of the name provided in AddResult method and config value "color" of ingredient (value)
		m_ResultToInventory[5] = -2;//(value) == -2 spawn result on the ground;(value) == -1 place anywhere in the players inventory, (value) >= 0 means switch position with ingredient number(value)
		m_ResultUseSoftSkills[5] = false;// set 'true' to allow modification of the values by softskills on this result
		m_ResultReplacesIngredient[5] = -1;// value == -1 means do nothing; a value >= 0 means this result will transfer item propertiesvariables, attachments etc.. from an ingredient value
	
		AddResult("egOCSds6");//add results here

		m_ResultSetFullQuantity[6] = false;//true = set full quantity, false = do nothing
		m_ResultSetQuantity[6] = 3;//-1 = do nothing
		m_ResultSetHealth[6] = -1;//-1 = do nothing
		m_ResultInheritsHealth[6] = 0;// (value) == -1 means do nothing; a (value) >= 0 means this result will inherit health from ingredient number (value);(value) == -2 means this result will inherit health from all ingredients averaged(result_health = combined_health_of_ingredients / number_of_ingredients)
		m_ResultInheritsColor[6] = -1;// (value) == -1 means do nothing; a (value) >= 0 means this result classname will be a composite of the name provided in AddResult method and config value "color" of ingredient (value)
		m_ResultToInventory[6] = -2;//(value) == -2 spawn result on the ground;(value) == -1 place anywhere in the players inventory, (value) >= 0 means switch position with ingredient number(value)
		m_ResultUseSoftSkills[6] = false;// set 'true' to allow modification of the values by softskills on this result
		m_ResultReplacesIngredient[6] = -1;// value == -1 means do nothing; a value >= 0 means this result will transfer item propertiesvariables, attachments etc.. from an ingredient value
	
		AddResult("egOCSds7");//add results here

		m_ResultSetFullQuantity[7] = false;//true = set full quantity, false = do nothing
		m_ResultSetQuantity[7] = 3;//-1 = do nothing
		m_ResultSetHealth[7] = -1;//-1 = do nothing
		m_ResultInheritsHealth[7] = 0;// (value) == -1 means do nothing; a (value) >= 0 means this result will inherit health from ingredient number (value);(value) == -2 means this result will inherit health from all ingredients averaged(result_health = combined_health_of_ingredients / number_of_ingredients)
		m_ResultInheritsColor[7] = -1;// (value) == -1 means do nothing; a (value) >= 0 means this result classname will be a composite of the name provided in AddResult method and config value "color" of ingredient (value)
		m_ResultToInventory[7] = -2;//(value) == -2 spawn result on the ground;(value) == -1 place anywhere in the players inventory, (value) >= 0 means switch position with ingredient number(value)
		m_ResultUseSoftSkills[7] = false;// set 'true' to allow modification of the values by softskills on this result
		m_ResultReplacesIngredient[7] = -1;// value == -1 means do nothing; a value >= 0 means this result will transfer item propertiesvariables, attachments etc.. from an ingredient value
	}
