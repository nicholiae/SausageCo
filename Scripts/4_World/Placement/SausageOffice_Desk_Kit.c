class SausageOffice_Desk_Kit extends ItemBase
{	
	ref protected EffectSound 						m_DeployLoopSound;
	protected Object								SausageOffice_Desk_Kit1;	
	...
	override void OnPlacementComplete(Man player, vector position = "0 0.4 0", vector orientation = "0 0 0")
	{
		super.OnPlacementComplete(player, position, orientation);
			...
			if (GetGame().IsServer())
			{
				EntityAI kitItem = EntityAI.Cast(GetGame().CreateObjectEx("SausageOffice_Desk", position, ECE_PLACE_ON_SURFACE));
				vector positionChange = "0 0.4 0";
...
class SausageOffice_Desk_Holo extends ItemBase
{
...
class SausageOffice_Desk extends ItemBase
{
	override bool CanPutInCargo( EntityAI parent )
    {
        if( !super.CanPutInCargo(parent) ) {return false;}        
