...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageLivingroom_EndTable) )
				{
					SausageLivingroom_EndTable sausagelivingroom_endtable = SausageLivingroom_EndTable.Cast(obj_collided);
					return sausagelivingroom_endtable;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageLivingroom_EndTable_Kit))
			return "SausageLivingroom_EndTable";
