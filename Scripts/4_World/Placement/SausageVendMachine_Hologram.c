...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageBuilding_VendMachine) )
				{
					SausageBuilding_VendMachine sausagebuilding_vendmachine = SausageBuilding_VendMachine.Cast(obj_collided);
					return sausagebuilding_vendmachine;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageBuilding_VendMachine_Kit))
			return "SausageBuilding_VendMachine";
