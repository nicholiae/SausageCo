...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageBuilding_SignKind) )
				{
					SausageBuilding_SignKind sausagebuilding_signkind = SausageBuilding_SignKind.Cast(obj_collided);
					return sausagebuilding_signkind;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageBuilding_SignKind_Kit))
			return "SausageBuilding_SignKind";
