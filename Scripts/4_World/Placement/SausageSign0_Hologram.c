...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageBuilding_Sign0) )
				{
					SausageBuilding_Sign0 sausagebuilding_sign0 = SausageBuilding_Sign0.Cast(obj_collided);
					return sausagebuilding_sign0;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageBuilding_Sign0_Kit))
			return "SausageBuilding_Sign0";
