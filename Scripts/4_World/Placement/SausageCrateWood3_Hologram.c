...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageCrateWood3_Storage) )
				{
					SausageCrateWood3_Storage sausagecratewood3_storage = SausageCrateWood3_Storage.Cast(obj_collided);
					return sausagecratewood3_storage;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageCrateWood3_Storage_Kit))
			return "SausageCrateWood3_Storage";
