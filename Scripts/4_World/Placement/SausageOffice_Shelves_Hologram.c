...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageOffice_Shelves) )
				{
					SausageOffice_Shelves sausageoffice_shelves = SausageOffice_Shelves.Cast(obj_collided);
					return sausageoffice_shelves;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageOffice_Shelves_Kit))
			return "SausageOffice_Shelves";
