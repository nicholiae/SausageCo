...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageCrateWood2_Storage) )
				{
					SausageCrateWood2_Storage sausagecratewood2_storage = SausageCrateWood2_Storage.Cast(obj_collided);
					return sausagecratewood2_storage;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageCrateWood2_Storage_Kit))
			return "SausageCrateWood2_Storage";
