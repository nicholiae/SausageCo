...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageBuilding_Well) )
				{
					SausageBuilding_Well sausagebase_well = SausageBuilding_Well.Cast(obj_collided);
					return sausagebase_well;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageBuilding_Well_Kit))
			return "SausageBuilding_Well";
