...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageKitchen_Cabinets) )
				{
					SausageKitchen_Cabinets sausagekitchen_cabinets = SausageKitchen_Cabinets.Cast(obj_collided);
					return sausagekitchen_cabinets;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageKitchen_Cabinets_Kit))
			return "SausageKitchen_Cabinets";
