...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageGarden_CompostBin) )
				{
					SausageGarden_CompostBin sausagegarden_compostbin = SausageGarden_CompostBin.Cast(obj_collided);
					return sausagegarden_compostbin;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageGarden_CompostBin_Kit))
			return "SausageGarden_CompostBin";
