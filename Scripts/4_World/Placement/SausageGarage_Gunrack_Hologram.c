...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageGarage_Gunrack) )
				{
					SausageGarage_Gunrack sausagegarage_gunrack = SausageGarage_Gunrack.Cast(obj_collided);
					return sausagegarage_gunrack;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageGarage_Gunrack_Kit))
			return "SausageGarage_Gunrack";
