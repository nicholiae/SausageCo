...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageBathroom_Cabinet) )
				{
					SausageBathroom_Cabinet sausagebathroom_cabinet = SausageBathroom_Cabinet.Cast(obj_collided);
					return sausagebathroom_cabinet;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageBathroom_Cabinet_Kit))
			return "SausageBathroom_Cabinet";
