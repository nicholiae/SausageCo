...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageBedroom_Hopechest) )
				{
					SausageBedroom_Hopechest sausagebedroom_hopechest = SausageBedroom_Hopechest.Cast(obj_collided);
					return sausagebedroom_hopechest;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageBedroom_Hopechest_Kit))
			return "SausageBedroom_Hopechest";
