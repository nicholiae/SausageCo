class SausageGarage_TrashCan_Kit extends ItemBase
{	
	ref protected EffectSound 						m_DeployLoopSound;
	protected Object								SausageGarage_TrashCan_Kit1;	
	...
	override void OnPlacementComplete( Man player )
	{
		...
		vector positionChange = "0 0.3 0";
		vector position += positionChange;
	#else
	override void OnPlacementComplete(Man player, vector position = "0 0.3 0", vector orientation = "0 0 0")
	{
		super.OnPlacementComplete(player, position, orientation);
	#endif
			if (GetGame().IsServer())
			{
				EntityAI kitItem = EntityAI.Cast(GetGame().CreateObjectEx("SausageGarage_TrashCan", position, ECE_PLACE_ON_SURFACE));
				vector positionChange = "0 0.3 0";
...
class SausageGarage_TrashCan_Holo extends ItemBase
{
...
class SausageGarage_TrashCan extends ItemBase
{
	override bool CanPutInCargo( EntityAI parent )
    {
        if( !super.CanPutInCargo(parent) ) {return false;}        
