...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageBuilding_Sign1) )
				{
					SausageBuilding_Sign1 sausagebuilding_sign1 = SausageBuilding_Sign1.Cast(obj_collided);
					return sausagebuilding_sign1;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageBuilding_Sign1_Kit))
			return "SausageBuilding_Sign1";
