...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageBedroom_Dressor) )
				{
					SausageBedroom_Dressor sausagebedroom_dressor = SausageBedroom_Dressor.Cast(obj_collided);
					return sausagebedroom_dressor;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageBedroom_Dressor_Kit))
			return "SausageBedroom_Dressor";
