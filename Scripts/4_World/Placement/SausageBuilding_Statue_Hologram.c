...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageBuilding_Statue) )
				{
					SausageBuilding_Statue sausagebuilding_statue = SausageBuilding_Statue.Cast(obj_collided);
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageBuilding_Statue_Kit))
			return "SausageBuilding_Statue";
