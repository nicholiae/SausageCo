...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageGarage_TrashCan) )
				{
					SausageGarage_TrashCan sausagegarage_trashcan = SausageGarage_TrashCan.Cast(obj_collided);
					return sausagegarage_trashcan;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageGarage_TrashCan_Kit))
			return "SausageGarage_TrashCan";
