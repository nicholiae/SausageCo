class SausageBuilding_Statue_Kit extends ItemBase
{	
	ref protected EffectSound 						m_DeployLoopSound;
	protected Object								SausageBuilding_Statue_Kit1;	
	...
	#else
	override void OnPlacementComplete(Man player, vector position = "0 3 0", vector orientation = "0 0 0")
	{
		super.OnPlacementComplete(player, position, orientation);
	#endif
			if (GetGame().IsServer())
			{
				EntityAI kitItem = EntityAI.Cast(GetGame().CreateObjectEx("SausageBuilding_Statue", position, ECE_PLACE_ON_SURFACE));
				vector positionChange = "0 3 0";
...
class SausageBuilding_Statue_Holo extends ItemBase
{
...
class SausageBuilding_Statue extends ItemBase
{
		override bool CanPutInCargo( EntityAI parent )
    {
        if( !super.CanPutInCargo(parent) ) {return false;}        
