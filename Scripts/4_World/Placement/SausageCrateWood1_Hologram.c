...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageCrateWood1_Storage) )
				{
					SausageCrateWood1_Storage sausagecratewood1_storage = SausageCrateWood1_Storage.Cast(obj_collided);
					return sausagecratewood1_storage;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageCrateWood1_Storage_Kit))
			return "SausageCrateWood1_Storage";
