...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageBathroom_Toilet) )
				{
					SausageBathroom_Toilet sausagebathroom_toilet = SausageBathroom_Toilet.Cast(obj_collided);
					return sausagebathroom_toilet;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageBathroom_Toilet_Kit))
			return "SausageBathroom_Toilet";
