...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageLights_Torch) )
				{
					SausageLights_Torch sausagebuilding_statue = SausageLights_Torch.Cast(obj_collided);
					return sausagebuilding_statue;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageLights_Torch_Kit))
			return "SausageLights_Torch";
