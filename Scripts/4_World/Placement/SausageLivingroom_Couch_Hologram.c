...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageLivingroom_Couch) )
				{
					SausageLivingroom_Couch sausagelivingroom_couch = SausageLivingroom_Couch.Cast(obj_collided);
					return sausagelivingroom_couch;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageLivingroom_Couch_Kit))
			return "SausageLivingroom_Couch";
