class SausageBuilding_Storage_Kit extends ItemBase
{	
	ref protected EffectSound 						m_DeployLoopSound;
	protected Object								SausageBuilding_Storage_Kit1;	
	...
	override void OnPlacementComplete(Man player, vector position = "0 0 0", vector orientation = "0 0 0")
	{
		super.OnPlacementComplete(player, position, orientation);
			...
			if (GetGame().IsServer())
			{
				EntityAI kitItem = EntityAI.Cast(GetGame().CreateObjectEx("SausageBuilding_Storage", position, ECE_PLACE_ON_SURFACE));
...
class SausageBuilding_Storage_Holo extends ItemBase
{
...
class SausageBuilding_Storage extends ItemBase
{
		override bool CanPutInCargo( EntityAI parent )
    {
        if( !super.CanPutInCargo(parent) ) {return false;}        
