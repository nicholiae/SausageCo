...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageBuilding_SmokeHouse) )
				{
					SausageBuilding_SmokeHouse sausagebuilding_smokehouse = SausageBuilding_SmokeHouse.Cast(obj_collided);
					return sausagebuilding_smokehouse;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageBuilding_SmokeHouse_Kit))
			return "SausageBuilding_SmokeHouse";
