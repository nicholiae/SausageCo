...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageBathroom_Sink) )
				{
					SausageBathroom_Sink sausagebathroom_sink = SausageBathroom_Sink.Cast(obj_collided);
					return sausagebathroom_sink;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageBathroom_Sink_Kit))
			return "SausageBathroom_Sink";
