...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageCo_CasingsProcessor) )
				{
					SausageCo_CasingsProcessor sausageco_casingsprocessor = SausageCo_CasingsProcessor.Cast(obj_collided);
					return sausageco_casingsprocessor;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageCo_CasingsProcessor_Kit))
			return "SausageCo_CasingsProcessor";
