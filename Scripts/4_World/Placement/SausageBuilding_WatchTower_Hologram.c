...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageBuilding_WatchTower) )
				{
					SausageBuilding_WatchTower sausagebuilding_watchtower = SausageBuilding_WatchTower.Cast(obj_collided);
					return sausagebuilding_watchtower;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageBuilding_WatchTower_Kit))
			return "SausageBuilding_WatchTower";
