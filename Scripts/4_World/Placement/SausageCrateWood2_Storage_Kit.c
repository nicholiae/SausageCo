class SausageCrateWood2_Storage_Kit extends ItemBase
{	
	ref protected EffectSound 						m_DeployLoopSound;
	protected Object								SausageCrateWood2_Storage_Kit1;	
	...
	override void OnPlacementComplete( Man player )
	{
		...
		vector positionChange = "0 0.6 0";
		position = position + positionChange;
		
	#else
	override void OnPlacementComplete(Man player, vector position = "0 0.6 0", vector orientation = "0 0 0")
	{
		super.OnPlacementComplete(player, position, orientation);
	#endif
			if (GetGame().IsServer())
			{
				EntityAI kitItem = EntityAI.Cast(GetGame().CreateObjectEx("SausageCrateWood2_Storage", position, ECE_PLACE_ON_SURFACE));
				vector positionChange = "0 0.6 0";
				position = position + positionChange;
...
class SausageCrateWood2_Storage_Holo extends ItemBase
{
...
class SausageCrateWood2_Storage extends ItemBase
{
		override bool CanPutInCargo( EntityAI parent )
    {
        if( !super.CanPutInCargo(parent) ) {return false;}        
