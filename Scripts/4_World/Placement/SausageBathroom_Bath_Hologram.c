...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageBathroom_Bath) )
				{
					SausageBathroom_Bath sausagebathroom_bath = SausageBathroom_Bath.Cast(obj_collided);
					return sausagebathroom_bath;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageBathroom_Bath_Kit))
			return "SausageBathroom_Bath";
