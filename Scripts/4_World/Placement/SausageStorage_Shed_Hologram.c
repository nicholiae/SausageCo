...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageStorage_Shed) )
				{
					SausageStorage_Shed sausagestorage_shed = SausageStorage_Shed.Cast(obj_collided);
					return sausagestorage_shed;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageStorage_Shed_Kit))
			return "SausageStorage_Shed";
