...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageBedroom_BunkBed) )
				{
					SausageBedroom_BunkBed sausagebedroom_bunkbed = SausageBedroom_BunkBed.Cast(obj_collided);
					return sausagebedroom_bunkbed;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageBedroom_BunkBed_Kit))
			return "SausageBedroom_BunkBed";
