...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageKitchen_Fridge) )
				{
					SausageKitchen_Fridge sausagekitchen_fridge = SausageKitchen_Fridge.Cast(obj_collided);
					return sausagekitchen_fridge;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageKitchen_Fridge_Kit))
			return "SausageKitchen_Fridge";
