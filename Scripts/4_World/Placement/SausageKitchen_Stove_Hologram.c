...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageKitchen_Stove) )
				{
					SausageKitchen_Stove sausagekitchen_stove = SausageKitchen_Stove.Cast(obj_collided);
					return sausagekitchen_stove;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageKitchen_Stove_Kit))
			return "SausageKitchen_Stove";
