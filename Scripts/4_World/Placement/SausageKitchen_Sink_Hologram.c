...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageKitchen_Sink) )
				{
					SausageKitchen_Sink sausagekitchen_sink = SausageKitchen_Sink.Cast(obj_collided);
					return sausagekitchen_sink;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageKitchen_Sink_Kit))
			return "SausageKitchen_Sink";
