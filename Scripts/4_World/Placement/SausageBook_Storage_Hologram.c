...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageBook_Storage) )
				{
					SausageBook_Storage sausagebook_storage = SausageBook_Storage.Cast(obj_collided);
					return sausagebook_storage;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageBook_Storage_Kit))
			return "SausageBook_Storage";
