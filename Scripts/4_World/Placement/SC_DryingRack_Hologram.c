...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SC_DryingRack) )
				{
					SC_DryingRack sc_dryingrack = SC_DryingRack.Cast(obj_collided);
					return sc_dryingrack;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SC_DryingRack_Kit))
			return "SC_DryingRack";
