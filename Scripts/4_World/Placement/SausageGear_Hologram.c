...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageGear_Storage) )
				{
					SausageGear_Storage sausagegear_storage = SausageGear_Storage.Cast(obj_collided);
					return sausagegear_storage;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageGear_Storage_Kit))
			return "SausageGear_Storage";
