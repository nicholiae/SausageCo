...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageHelipad_Single) )
				{
					SausageHelipad_Single sausagehelipad_single = SausageHelipad_Single.Cast(obj_collided);
					return sausagehelipad_single;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageHelipad_Single_Kit))
			return "SausageHelipad_Single";
