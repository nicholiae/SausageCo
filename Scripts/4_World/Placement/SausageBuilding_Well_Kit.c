class SausageBuilding_Well_Kit extends ItemBase
{	
	ref protected EffectSound 						m_DeployLoopSound;
	protected Object								SausageBuilding_Well_Kit1;	
	...
	override void OnPlacementComplete( Man player )
	{
		...
		vector positionChange = "0 0.2 0";
		vector position += positionChange;
	#else
	override void OnPlacementComplete(Man player, vector position = "0 0.2 0", vector orientation = "0 0 0")
	{
		super.OnPlacementComplete(player, position, orientation);
	#endif
			if (GetGame().IsServer())
			{
				EntityAI kitItem = EntityAI.Cast(GetGame().CreateObjectEx("SausageBuilding_Well", position, ECE_PLACE_ON_SURFACE));
				vector positionChange = "0 0.2 0";
...
class SausageBuilding_Well_Holo extends ItemBase
{
...
class SausageBuilding_Well extends ItemBase
{
	override bool CanPutInCargo( EntityAI parent )
    {
        return false;
