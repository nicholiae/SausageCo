...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageBedroom_NightStand) )
				{
					SausageBedroom_NightStand sausagebedroom_nightstand = SausageBedroom_NightStand.Cast(obj_collided);
					return sausagebedroom_nightstand;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageBedroom_NightStand_Kit))
			return "SausageBedroom_NightStand";
