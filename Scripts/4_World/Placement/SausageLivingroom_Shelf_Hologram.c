...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageLivingroom_Shelf) )
				{
					SausageLivingroom_Shelf sausagelivingroom_shelf = SausageLivingroom_Shelf.Cast(obj_collided);
					return sausagelivingroom_shelf;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageLivingroom_Shelf_Kit))
			return "SausageLivingroom_Shelf";
