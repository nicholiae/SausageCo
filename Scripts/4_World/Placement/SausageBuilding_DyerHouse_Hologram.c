...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageBuilding_DyerHouse) )
				{
					SausageBuilding_DyerHouse sausagebuilding_dyerhouse = SausageBuilding_DyerHouse.Cast(obj_collided);
					return sausagebuilding_dyerhouse;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageBuilding_DyerHouse_Kit))
			return "NewSC_Prefab1";
