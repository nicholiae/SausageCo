...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausagePlank_Processor) )
				{
					SausagePlank_Processor sausageplank_processor = SausagePlank_Processor.Cast(obj_collided);
					return sausageplank_processor;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausagePlank_Processor_Kit))
			return "SausagePlank_Processor";
