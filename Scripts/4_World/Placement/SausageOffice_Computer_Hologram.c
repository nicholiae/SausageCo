...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageOffice_Computer) )
				{
					SausageOffice_Computer sausageoffice_computer = SausageOffice_Computer.Cast(obj_collided);
					return sausageoffice_computer;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageOffice_Computer_Kit))
			return "SausageOffice_Computer";
