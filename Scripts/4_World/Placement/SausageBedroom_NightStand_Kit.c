class SausageBedroom_NightStand_Kit extends ItemBase
{	
	ref protected EffectSound 						m_DeployLoopSound;
	protected Object								SausageBedroom_NightStand_Kit1;	
	...
	override void OnPlacementComplete( Man player )
	{
		...
		vector positionChange = "0 0.4 0";
		vector position += positionChange;
	#else
	override void OnPlacementComplete(Man player, vector position = "0 0.4 0", vector orientation = "0 0 0")
	{
		super.OnPlacementComplete(player, position, orientation);
	#endif
			if (GetGame().IsServer())
			{
				EntityAI kitItem = EntityAI.Cast(GetGame().CreateObjectEx("SausageBedroom_NightStand", position, ECE_PLACE_ON_SURFACE));
				vector positionChange = "0 0.4 0";
...
class SausageBedroom_NightStand_Holo extends ItemBase
{
...
class SausageBedroom_NightStand extends ItemBase
{
	override bool CanPutInCargo( EntityAI parent )
    {
        if( !super.CanPutInCargo(parent) ) {return false;}        
