...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageOffice_Chair) )
				{
					SausageOffice_Chair sausageoffice_chair = SausageOffice_Chair.Cast(obj_collided);
					return sausageoffice_chair;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageOffice_Chair_Kit))
			return "SausageOffice_Chair";
