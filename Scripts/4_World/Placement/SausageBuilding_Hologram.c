...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageBuilding_Storage) )
				{
					SausageBuilding_Storage sausagebuilding_storage = SausageBuilding_Storage.Cast(obj_collided);
					return sausagebuilding_storage;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageBuilding_Storage_Kit))
			return "SausageBuilding_Storage";
