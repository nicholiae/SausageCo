...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageOffice_Desk) )
				{
					SausageOffice_Desk sausageoffice_desk = SausageOffice_Desk.Cast(obj_collided);
					return sausageoffice_desk;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageOffice_Desk_Kit))
			return "SausageOffice_Desk";
