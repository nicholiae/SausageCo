class SausageBuilding_DyerHouse_Kit extends ItemBase
{	
	ref protected EffectSound 						m_DeployLoopSound;
	protected Object								SausageBuilding_DyerHouse_Kit1;	
	...
	override void OnPlacementComplete( Man player )
	{
		...
		vector positionChange = "0 2.25 0";
		vector position += positionChange;
	#else
	override void OnPlacementComplete(Man player, vector position = "0 2.25 0", vector orientation = "0 0 0")
	{
		super.OnPlacementComplete(player, position, orientation);
	#endif
			if (GetGame().IsServer())
			{
				EntityAI kitItem = EntityAI.Cast(GetGame().CreateObjectEx("NewSC_Prefab1", position, ECE_PLACE_ON_SURFACE));
				vector positionChange = "0 2.25 0";
...
class SausageBuilding_DyerHouse_Holo extends ItemBase
{
...
class SausageBuilding_DyerHouse extends ItemBase
{
		override bool CanPutInCargo( EntityAI parent )
    {
        if( !super.CanPutInCargo(parent) ) {return false;}        
