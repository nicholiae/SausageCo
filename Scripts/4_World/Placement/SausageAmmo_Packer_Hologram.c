...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageAmmo_Packer) )
				{
					SausageAmmo_Packer sausageammo_packer = SausageAmmo_Packer.Cast(obj_collided);
					return sausageammo_packer;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageAmmo_Packer_Kit))
			return "SausageAmmo_Packer";
