...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SC_SolarPanel) )
				{
					SC_SolarPanel sc_solarpanel = SC_SolarPanel.Cast(obj_collided);
					return sc_solarpanel;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SC_SolarPanel_Kit))
			return "SC_SolarPanel";
