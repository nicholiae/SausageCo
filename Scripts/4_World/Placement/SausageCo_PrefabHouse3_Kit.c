class SausageCo_PrefabHouse3_Kit extends ItemBase
{	
	ref protected EffectSound 						m_DeployLoopSound;
	protected Object								SausageCo_PrefabHouse3_Kit1;	
	...
	override void OnPlacementComplete(Man player, vector position = "0 0 0", vector orientation = "0 0 0")
	{
		super.OnPlacementComplete(player, position, orientation);
			...
			if (GetGame().IsServer())
			{
				EntityAI kitItem = EntityAI.Cast(GetGame().CreateObjectEx("SausageCo_PrefabHouse3", position, ECE_PLACE_ON_SURFACE));
...
class SausageCo_PrefabHouse3_Holo extends ItemBase
{
...
class SausageCo_PrefabHouse3 extends ItemBase
{
	override bool CanPutInCargo( EntityAI parent )
    {
        if( !super.CanPutInCargo(parent) ) {return false;}        
