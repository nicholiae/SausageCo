...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageDecor_Picture1) )
				{
					SausageDecor_Picture1 sausagedecor_picture1 = SausageDecor_Picture1.Cast(obj_collided);
					return sausagedecor_picture1;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageDecor_Picture1_Kit))
			return "SausageDecor_Picture1";
