class SausageGarage_Gunrack_Kit extends ItemBase
{	
	ref protected EffectSound 						m_DeployLoopSound;
	protected Object								SausageGarage_Gunrack_Kit1;	
	...
	override void OnPlacementComplete(Man player, vector position = "0 0.4 0", vector orientation = "0 0 0")
	{
		super.OnPlacementComplete(player, position, orientation);
			...
			if (GetGame().IsServer())
			{
				EntityAI kitItem = EntityAI.Cast(GetGame().CreateObjectEx("SausageGarage_Gunrack", position, ECE_PLACE_ON_SURFACE));
				vector positionChange = "0 0.4 0";
...
class SausageGarage_Gunrack_Holo extends ItemBase
{
...
class SausageGarage_Gunrack extends ItemBase
{
	override bool CanPutInCargo( EntityAI parent )
    {
        if( !super.CanPutInCargo(parent) ) {return false;}        
