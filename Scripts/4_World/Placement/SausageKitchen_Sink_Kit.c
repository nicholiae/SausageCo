class SausageKitchen_Sink_Kit extends ItemBase
{	
	ref protected EffectSound 						m_DeployLoopSound;
	protected Object								SausageKitchen_Sink_Kit1;	
	...
	override void OnPlacementComplete(Man player, vector position = "0 0 0", vector orientation = "0 0 0")
	{
		super.OnPlacementComplete(player, position, orientation);
			...
			if (GetGame().IsServer())
			{
				EntityAI kitItem = EntityAI.Cast(GetGame().CreateObjectEx("SausageKitchen_Sink", position, ECE_PLACE_ON_SURFACE));
...
class SausageKitchen_Sink_Holo extends ItemBase
{
...
class SausageKitchen_Sink extends ItemBase
{
	override bool CanPutInCargo( EntityAI parent )
    {
        return false;
