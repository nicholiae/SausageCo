...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageBuilding_GasPump) )
				{
					SausageBuilding_GasPump sausagebuilding_gaspump = SausageBuilding_GasPump.Cast(obj_collided);
					return sausagebuilding_gaspump;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageBuilding_GasPump_Kit))
			return "SausageBuilding_GasPump";
