...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageLivingroom_CoffeeTable) )
				{
					SausageLivingroom_CoffeeTable sausagelivingroom_coffeetable = SausageLivingroom_CoffeeTable.Cast(obj_collided);
					return sausagelivingroom_coffeetable;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageLivingroom_CoffeeTable_Kit))
			return "SausageLivingroom_CoffeeTable";
