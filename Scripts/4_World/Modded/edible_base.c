...
{	
	...
	{
		super.EEInit();
		m_PreservationMultiplier = GetSCSettingsConfig().Fridge_Settings.PreservationMultiplier;
		m_EnableFridgePreservation = GetSCSettingsConfig().Fridge_Settings.EnableFridgePreservation;
	}
	#endif
	...
	override void ProcessDecay( float delta, bool hasRootAsPlayer )
	{
		if(m_EnableFridgePreservation)
		{
			...
			{				
				if(SC_Helper.IsFridgeOn(parent) && m_PreservationMultiplier > 0)
				{
...
#endif
    void SC_SetDecayTimer(float newTimer)
    {
        m_DecayTimer = newTimer;
    }

    void SC_SetLastDecayStage(FoodStageType newStage)
    {
