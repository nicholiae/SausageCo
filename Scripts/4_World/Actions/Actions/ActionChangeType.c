class ActionChangeType: ActionInteractBase
{
	...
	override bool ActionCondition( PlayerBase player, ActionTarget target, ItemBase item )
	{
		...
		SC_ItemBase itemSC = SC_ItemBase.Cast( targetObject );
		if (itemSC && player)
		{
			string newItemName = itemSC.GetNextInterchangeableItem();
			if(newItemName == "")
			{
				return false;
			}
	...
	override void OnExecuteServer( ActionData action_data )
	{
		...
		SC_ItemBase item = SC_ItemBase.Cast( targetObject );
		if(item)
		{
			...
			SC_ItemBase newItem = SC_ItemBase.Cast(GetGame().CreateObject(newItemName, itemPos));
	...
	void MoveItems(SC_ItemBase oldItem, SC_ItemBase newItem)
	{		
