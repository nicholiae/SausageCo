...
class ActionFoldItem : ActionContinuousBase
{
	...
	override bool ActionCondition(PlayerBase player, ActionTarget target, ItemBase item)
	{
		...
		{
			SC_Foldable_Item foldable_item = SC_Foldable_Item.Cast(target.GetObject());
	...
	{
		SC_Foldable_Item foldable_item = SC_Foldable_Item.Cast(action_data.m_Target.GetObject());
		if (foldable_item)
			foldable_item.Fold();
	}
