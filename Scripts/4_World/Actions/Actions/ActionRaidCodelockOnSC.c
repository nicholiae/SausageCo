#ifdef CodeLock
class ActionRaidCodelockOnSCCB : ActionContinuousBaseCB
{
    override void CreateActionComponent()
    {
        float raidTime = 15;
        if(GetSausageCompanyCodelockConfig())
        {
            raidTime = GetSausageCompanyCodelockConfig().Get_RaidTimeInSeconds() / GetDayZGame().GetCodeLockConfig().GetIncrementAmount(); // config value to minutes
        }
...
class ActionRaidCodelockOnSC : ActionContinuousBase
{
    protected float m_LockHealth;
    protected float m_MaxHealth;

    void ActionRaidCodelockOnSC()
    {
        m_CallbackClass = ActionRaidCodelockOnSCCB;
    ...
    override bool ActionCondition(PlayerBase player, ActionTarget target, ItemBase item)
    {
        ...
        if (itemBase && (itemBase.IsKindOf("SC_Openable_Placeable_Base")|| itemBase.IsKindOf("SC_Openable_Base") || itemBase.IsKindOf("SC_Greenhouse_Base")))
        {
            if(!GetSausageCompanyCodelockConfig())
            {
                return false;
            ...
            array<string> raidTools = GetSausageCompanyCodelockConfig().Get_RaidTools();
            if(raidTools.Find(itemInHands.GetType()) == -1)
                return false;

            Class.CastTo(codelock, itemBase.GetCodeLock());

            if (codelock && GetSausageCompanyCodelockConfig().CanRaid()) 
            {
    ...
    override void OnFinishProgressServer(ActionData action_data)
    {
        ...
        if (!GetSausageCompanyCodelockConfig().CanRaid())
        {
            return;
        }
        ...
        int toolDamage = GetSausageCompanyCodelockConfig().Get_ToolDamageOnRaid();
        ...
        if (itemBase)
        {
            ...
            if (codelock) 
            {
                ...
                else 
                {
                    ...
                    if(SCitembase && SCitembase.IsSCVirtualStorageEnabled())
                    {
                        SCitembase.RestoreSCInventory(action_data.m_Player);
                    }
                    #endif

                    if (GetSausageCompanyCodelockConfig().ShouldDeleteLockOnRaid())
                        codelock.Delete();                        
                    if (toolDamage > 0)
