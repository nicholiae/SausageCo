...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageFurnace) )
				{
					SausageFurnace sausagefurnace = SausageFurnace.Cast(obj_collided);
					return sausagefurnace;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageFurnace_Kit))
			return "SausageFurnace";
