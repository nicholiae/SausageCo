class SausageAnvil_Kit extends ItemBase
{	
	ref protected EffectSound 						m_DeployLoopSound;
	protected Object								SausageAnvil_Kit1;	
	...
	override void OnPlacementComplete(Man player, vector position = "0 0 0", vector orientation = "0 0 0")
	{
		super.OnPlacementComplete(player, position, orientation);
			...
			if (GetGame().IsServer())
			{
				EntityAI kitItem = EntityAI.Cast(GetGame().CreateObjectEx("SausageAnvil", position, ECE_PLACE_ON_SURFACE));
...
class SausageAnvil_Holo extends ItemBase
{
...
class SausageAnvil extends ItemBase
{
		override bool CanPutInCargo( EntityAI parent )
    {
        if( !super.CanPutInCargo(parent) ) {return false;}        
...
class SausageAnvil_Static extends SausageAnvil
{
