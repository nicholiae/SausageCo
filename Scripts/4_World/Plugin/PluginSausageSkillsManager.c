...
class PluginSausageSkillsManager extends PluginBase
{
    ...
    override void OnInit()
    {
        ...
        if (GetGame().IsServer())
        {
            ...
            GetGame().GetRPCManager().AddRPC("SausageSkills", "RequestSkillsData", this, SingleplayerExecutionType.Server);
            GetGame().GetRPCManager().AddRPC("SausageSkills", "CraftRecipe", this, SingleplayerExecutionType.Server);
        }
    }
    ...
    void SyncSkillsToClient(PlayerBase player)
    {
        ...
        if (skillsData)
        {
            ...
            GetGame().GetRPCManager().SendRPC("SausageSkills", "SyncPlayerSkills", new Param1<ref array<ref Param3<string, int, int>>>(skillsArray), true, player.GetIdentity());
    ...
    void SyncSkillToClient(PlayerBase player, string skillType, int level, int experience)
    {
        ...
        GetGame().GetRPCManager().SendRPC("SausageSkills", "UpdateSkill", new Param3<string, int, int>(skillType, level, experience), true, player.GetIdentity());
    }
    ...
    void SendMessageToClient(PlayerBase player, string message)
    {
        ...
        {
            // Use the global GetRPCManager() function to avoid conflicts
            GetGame().GetRPCManager().SendRPC("SausageSkills", "DisplayMessage", new Param1<string>(message), true, player.GetIdentity());
        }
    }
