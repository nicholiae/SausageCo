...
{	
	ItemBase FadedCollision()
	{
		...
		{	
			for( int i = 0; i < collided_objects.Count(); i++ )
			{
				...
				if( obj_collided.IsInherited(SausageWeed_Processor) )
				{
					SausageWeed_Processor sausageweed_processor = SausageWeed_Processor.Cast(obj_collided);
					return sausageweed_processor;
				}
			}
		}
		return null;
	...
	override string ProjectionBasedOnParent()
	{
		...
		if (item_in_hands.IsInherited(SausageWeed_Processor_Kit))
			return "SausageWeed_Processor";
