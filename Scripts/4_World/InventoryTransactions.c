//This was created by Nicholiae to solve the CreateInInventory issues
//All rights reserved
//Redistribution of this code cannot be done without explicit permission from Nicholiae
enum SC_InventorySearch
{
    Attachments,
    DeepSearch
};

class SC_InventoryTransactions
{
    ...
    static EntityAI CreateItemInPlayerInventory(string classname, notnull PlayerBase target, out InventoryLocationType locationType, bool createOnGroundOnFailure = true, SC_InventorySearch SearchType = SC_InventorySearch.Attachments)
    {        
        #ifdef SERVER
        //create item and
        ...
        EntityAI newItem = EntityAI.Cast(SC_InventoryTransactions.TryMoveLocalItem(newItemLocal, target, locationType));
        if (newItem)
        {
            return newItem;
        }
        else
        {
            switch (SearchType)
            {
            case SC_InventorySearch.Attachments:
                for (int i = 0; i < target.GetInventory().GetAttachmentSlotsCount(); i++)
                {
                    ...
                    if (attachment)
                    {
                        ...
                        newItem = EntityAI.Cast(SC_InventoryTransactions.TryMoveLocalItem(newItemLocal, attachment, locationType));
                        if (newItem)
                        {
                            return newItem;
                        }
                ...
                break;
            case SC_InventorySearch.DeepSearch:
                ...
                foreach (EntityAI child : children)
                {
                    if (child)
                    {
                        ...
                        newItem = EntityAI.Cast(SC_InventoryTransactions.TryMoveLocalItem(newItemLocal, child, locationType));
                        if (newItem)
                        {
                            return newItem;
                        }
