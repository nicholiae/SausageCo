#ifdef SERVER
static void SausageCompany_Print(string message)
{
...
{    
	...
	static bool IsFridgeOn(EntityAI parent)
	{
		bool isFridge = false;
		EntityAI fridge;
		if(parent.IsInherited(SC_Fridge_Base))
		{
			isFridge = true;
			fridge = parent;
		}
		...
		if(!fridge)
		{
			array<string> OtherFridges = GetSCSettingsConfig().Fridge_Settings.OtherFridgeClasses;
