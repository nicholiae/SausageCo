...
class ActionRaidRACodeLockOnSCCB : ActionContinuousBaseCB
{
    override void CreateActionComponent()
    {
        ...
        {
            raidTime = GetSausageCompanyCodelockConfig().Get_RaidTimeInSeconds() / 5; // config value to minutes
        }
...
class ActionRaidRACodeLockOnSC : ActionContinuousBase
{

    void ActionRaidRACodeLockOnSC()
    {
        m_CallbackClass = ActionRaidRACodeLockOnSCCB;
    ...
    override bool ActionCondition(PlayerBase player, ActionTarget target, ItemBase item)
    {
        ...
        if (itemBase && (itemBase.IsKindOf("SC_Openable_Placeable_Base")|| itemBase.IsKindOf("SC_Openable_Base") || itemBase.IsKindOf("SC_Greenhouse_Base")))
        {
            ...
            if (code_lock && GetSausageCompanyCodelockConfig().CanRaid()) 
            {
                return true;
            }
        }       
