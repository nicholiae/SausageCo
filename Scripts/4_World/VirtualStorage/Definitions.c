...
class SC_Energy_State
{
	float EnergyAmount;
};

class SC_Cartridge_Info
{
	int infoIndex;
	string className;
	float damage;
...
class SC_Cartridge_Info_List
{
	ref array<ref SC_Cartridge_Info> list = new array<ref SC_Cartridge_Info>;
};

class SC_Weapon_State
{
	...
	bool isJammed = false;
	ref map<int, ref SC_Cartridge_Info> chamberCartridgeInfo = new map<int, ref SC_Cartridge_Info>;
	ref map<int, ref SC_Cartridge_Info_List> intMagCrtrdgInfo = new map<int, ref SC_Cartridge_Info_List>;
};

class SC_Magazine_State
{
	int ammoCount = -1;
	ref array<ref SC_Cartridge_Info> magCrtrdgInfo = new array<ref SC_Cartridge_Info>;
};

class SC_Item_CustomVariables
{
	ref map<string, float> iteSCloatVariables = new map<string, float>;
...
class SC_Container
{
	...
	ref SC_Item_CustomVariables customVariables;
	ref array<ref SC_Inventory> attachments = new array<ref SC_Inventory>;
	ref array<ref SC_Inventory> cargo = new array<ref SC_Inventory>;
};
...
class SC_Inventory
{
	...
	bool flipped;
	int agents = 0;
	SC_Item_Type itemType = SC_Item_Type.Regular;
	ref SC_Weapon_State wpnState;
	ref SC_Magazine_State magState;
	ref SC_Energy_State energyState;
	ref SC_Food_State foodState;
	...
	void SC_Inventory(bool storeCargo = true)
	{
	...
	bool StoreEntity(EntityAI entity) 
	{
		...
		{
			itemType = SC_Item_Type.Magazine;
			Magazine_Base mag = Magazine_Base.Cast(m_Entity);
			if(mag)
			{
				magState = new SC_Magazine_State();
				...
				for (int ammoIndex = 0; ammoIndex < mag.GetAmmoCount(); ammoIndex++)
				{
					...
					ref SC_Cartridge_Info chamberinfo = new SC_Cartridge_Info();
		...
		{
			itemType = SC_Item_Type.Weapon;
			StoreWeapon();
		}

        if(m_Entity.IsAmmoPile())
        {
            itemType = SC_Item_Type.Ammo;			
		...
		{
            itemType = SC_Item_Type.Energy;	
			energyState = new SC_Energy_State();
			energyState.EnergyAmount = m_Entity.GetCompEM().GetEnergy();
		}

        if(m_Entity.IsKindOf("Edible_Base"))
        {
            itemType = SC_Item_Type.Food;
			foodState = new SC_Food_State();
	...
	EntityAI CreateSubEntity(notnull EntityAI parent)
	{
		...
		if(itemType == SC_Item_Type.Magazine)
		{
			Weapon_Base wpn = Weapon_Base.Cast(parent);
			if(wpn)
			{
				EntityAI magAtt = EntityAI.Cast(wpn.SC_SpawnAttachedMagazine(typeName, false));
				if(magAtt)
				{
					wpn.ForceSyncSelectionState();
					return magAtt;
				}
				Error(string.Format("SausageFramework: Couldn't create weapon magazine: %1 for weapon %2",typeName, Object.GetDebugName(parent)));
			}
		}
		switch (locationType) 
		{
			case InventoryLocationType.ATTACHMENT:
				...
				if(attachment)
				{
					//Print(string.Format("SausageFramework: Created attachment: %1 at index %2",typeName, index));
					return attachment;
				}	
				//Print(string.Format("SausageFramework: Couldn't create attachment: %1 at index %2",typeName, index));
			break;
			case InventoryLocationType.CARGO:
				EntityAI cargoItem = parent.GetInventory().CreateEntityInCargoEx(typeName, index, row, col, flipped);
				if(cargoItem)
				{
					//Print(string.Format("SausageFramework: Created entity in cargo: %1 at index %2 row %3 col %4 flipped %5",typeName, index, row, col, flipped));
					return cargoItem;
				}
				//Print(string.Format("SausageFramework: Couldn't create entity in cargo: %1 at index %2 row %3 col %4 flipped %5",typeName, index, row, col, flipped));
		}
	...
	{	
		...
		{				
			...
			if(mag)
			{
				...
				foreach (ref SC_Cartridge_Info mag_info : magState.magCrtrdgInfo)
				{						
		...
		if(itemType == SC_Item_Type.Weapon)
		{
			RestoreWeapon();
		}

		if(itemType == SC_Item_Type.Energy)
		{
			m_Entity.GetCompEM().SetEnergy(energyState.EnergyAmount);
		}

		if(itemType == SC_Item_Type.Food)
		{
			Edible_Base edible = Edible_Base.Cast(m_Entity);
			if(edible)
			{
				...
				edible.SC_SetLastDecayStage(foodState.LastStageType);
				edible.SC_SetDecayTimer(foodState.DecayTimer);
			}
		}
	...
	protected bool StoreAttachments() 
	{
		...
		for (int i = 0; i < attCount; ++i) 
		{
			...
			SC_Inventory attInv = new SC_Inventory(m_StoreCargo);
			if (!attInv.StoreEntity(att))
				return false;
			
			// #ifdef DIAG_DEVELOPER
			// Print(string.Format("SC_Virtual: Entity: %1 added attachment %2", Object.GetDebugName(m_Entity), Object.GetDebugName(att)));
			// #endif
	...
	protected bool StoreCargo() 
	{
		if(m_Entity.GetInventory() && m_Entity.GetInventory().GetCargo())
		{
			...
			for (int i = 0; i < attCount; ++i) 
			{
				...
				SC_Inventory attInv = new SC_Inventory();
	...
	protected void UniqueCargoInsert(SC_Inventory inv)
	{
		foreach(SC_Inventory storedCargo : cargo)
		{
			if(storedCargo)
			{
		...
		if(wpn)
		{
			...
			for (int muzzleIndex = 0; muzzleIndex < wpnState.mode_count; ++muzzleIndex)
			{
				...
				if (!wpn.IsChamberEmpty(muzzleIndex))
				{
					...
					{						
						SC_Cartridge_Info chamberinfo = new SC_Cartridge_Info();
				...
				{
					SC_Cartridge_Info_List info_list = new SC_Cartridge_Info_List();
					for (int cartridgeIndex = 0; cartridgeIndex < wpn.GetInternalMagazineCartridgeCount(muzzleIndex); ++cartridgeIndex)
					{
						...
						{
							SC_Cartridge_Info info = new SC_Cartridge_Info;
	...
	protected void RestoreWeapon()
	{
		...
		if(wpn)
		{
			...
			{
				ref SC_Cartridge_Info_List info_list = new SC_Cartridge_Info_List();
				if(wpnState.intMagCrtrdgInfo.Find(crtIndex, info_list) != -1)
				{
					if(info_list && info_list.list)
					{
						foreach(SC_Cartridge_Info intMagInfo : info_list.list)
						{
	...
	protected void InitializeCustomVariables()
	{
		if(!customVariables)
		{	
			customVariables = new SC_Item_CustomVariables();
		}
	}

	protected SC_Weapon_State GetWeaponState()
	{
		if(wpnState)
		{
			return wpnState;
